...
class NormalProgramHandler:
    def __init__(self, terms, facts, subdoms):
        ...
        self.__provability_dict = {}  # [{'a(1,1)' : [['r1_posbody1_ok(1)', 'r1_posbody2_ok(1,1)'], ['r1_posbody1_ok(1), 'r1_posbody2_ok(1,2)']]},
        # {'a(1,2)' : ['r1_posbody1_ok(1)', 'r1_posbody2_ok(2,1)', 'r1_posbody1_ok(1)', 'r1_posbody2_ok(2,2)']}]
    ...
    def __ensure_provability_positive_body_atom(self, f_interpretation, f_rem_atoms,
        ...
        pos_body_ok = f"r{rule_counter}_posbody{cur_func.index(body_literal)}_" \
                      f"ok({self.__get_joined_arguments(f_interpretation)})"
    ...
    def __ensure_provability_negative_body_atom(self, f_interpretation, f_rem_atoms,
        ...
        neg_body_ok = f"r{rule_counter}_negbody{cur_func.index(body_literal)}_" \
                      f"ok({self.__get_joined_arguments(f_interpretation)})"
    ...
    def __derive_provability_head(self, ground_head, non_ground_head, cur_var, cur_func):
        ...
        for c in provable_combs:
            same_variables_counter = 0
            same_values_counter = 0
            for body in cur_var:
                if cur_var.index(body) != 0:
                    ...
                    for h_arg in h_args:
                        ...
                        for b_arg in b_args:
                            if h_arg == b_arg and h_arg in cur_func:
                                same_variables_counter += 1
                                body_index = cur_var.index(body)
    ...
    def derive_provability_fact(self, node, cur_var, cur_func, g_counter):
        ...
        if self.__is_in_facts(head):
            ...
            self.__derive_provability_ground_head(cur_func, g_counter)
    ...
    def __derive_provability_ground_head(self, cur_func, g_counter):
        for body_atom in cur_func:
            if body_atom != cur_func[0]:
                if str(body_atom).startswith("not "):
                    neg = ""
                else:
                    neg = "not "
                self.ensure_justifiability_normal_programs(neg + str(body_atom), [], g_counter, body_atom, cur_func)
